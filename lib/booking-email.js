...
/**
 * Sends an official cancellation email to the guest when an admin cancels a booking.
 * Explicitly fulfills policy: Does not display specific deposited amounts, mentions manual refund within 2-3 days upon verification.
 * 
 * @param {object} params
 * @param {object} params.booking - The booking record
 * @param {string} [params.cancelledBy] - Admin/Staff email who performed cancellation
 * @param {string} [params.reason] - Optional cancellation reason
 */
export async function sendBookingCancellationEmail({ booking, cancelledBy = 'Resort Administration', reason = '' }) {
  if (!booking?.email) {
    console.warn(`[EMAIL:CANCEL_SKIPPED] Booking ${booking?.id} has no email address.`)
    return { sent: false, reason: 'no-guest-email' }
  }

  const { user, appPassword, owners } = getMailConfig()
  if (!user || !appPassword) {
    console.warn(`[EMAIL:CANCEL_SKIPPED] Gmail credentials not configured for booking ${booking?.id}`)
    return { sent: false, reason: 'gmail-not-configured' }
  }
  ...
  const dateLabel = isEvent ? 'Event Date' : (isShortStay ? 'Stay Date (Day Use)' : (isDayTour ? 'Visit Date' : 'Check-in Date'))
  const dateValue = date(booking.check_in)
...
<head>
  ...
  <title>Booking Cancelled - ${RESORT_NAME}</title>
</head>
<body style="margin:0;padding:24px 12px;background:#f3f5ef;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;color:#1f2937">
  <div style="max-width:600px;margin:auto;background:#ffffff;border:1px solid #dbe4d7;border-radius:16px;overflow:hidden;box-shadow:0 6px 20px rgba(0,0,0,0.06)">
    
    <!-- Brand Header -->
    <div style="padding:28px 24px;background:#173d35;color:#ffffff;text-align:center">
      <div style="font-size:11px;font-weight:700;letter-spacing:1.8px;text-transform:uppercase;color:#d5b36a">Siddhi Farm Resort</div>
      <h1 style="margin:8px 0 0;font-size:22px;font-weight:700;color:#ffffff">Reservation Cancellation Notice</h1>
      <p style="margin:4px 0 0;font-size:13px;color:#dbe4d7">Booking ID: <strong style="color:#d5b36a">${safe(booking.id)}</strong></p>
    </div>

    <!-- Main Content Body -->
    <div style="padding:28px 24px">
      <p style="margin:0 0 16px;font-size:15px;color:#173d35;line-height:1.6">
        Dear <strong>${safe(booking.name || 'Guest')}</strong>,
      </p>
      <p style="margin:0 0 18px;font-size:14px;color:#334155;line-height:1.6">
        Please be informed that your booking with <strong>${RESORT_NAME}</strong> has been cancelled by resort administration.
      </p>

      ${reason ? `<div style="margin:16px 0;padding:12px 16px;background:#f8fafc;border-left:4px solid #94a3b8;border-radius:4px;font-size:13px;color:#475569">
        <strong>Reason for cancellation:</strong> ${safe(reason)}
      </div>` : ''}

      <!-- Refund Policy Notice (Mandatory Wording) -->
      <div style="margin:22px 0;padding:18px 20px;background:#fefbf2;border:1px solid #e9d7a5;border-radius:12px">
        <div style="display:flex;align-items:center;gap:8px;font-size:13px;font-weight:700;color:#925f0a;text-transform:uppercase;letter-spacing:0.8px">
          ℹ️ Payment &amp; Refund Notice
        </div>
        <p style="margin:8px 0 0;font-size:13px;color:#5a4918;line-height:1.6">
          If you have already deposited or paid any advance or full payment for this booking, please be assured that the amount will be refunded after verification. 
        </p>
        <p style="margin:8px 0 0;font-size:13px;color:#5a4918;line-height:1.6">
          Refunds are generated manually by our accounts team and will be processed back to your original source of payment within <strong>2 to 3 business days</strong>.
        </p>
      </div>

      <!-- Cancelled Booking Summary -->
      <h3 style="margin:24px 0 10px;font-size:14px;text-transform:uppercase;letter-spacing:1px;color:#173d35;font-weight:700">Cancelled Booking Summary</h3>
      <table style="width:100%;border-collapse:collapse;margin-bottom:20px;background:#fafcf9;border:1px solid #e3ebe0;border-radius:8px;overflow:hidden">
        <tbody>
          <tr>
            <td style="padding:10px 14px;border-bottom:1px solid #e3ebe0;color:#5b6a63;font-size:13px;width:40%">Booking ID</td>
            <td style="padding:10px 14px;border-bottom:1px solid #e3ebe0;font-weight:600;color:#173d35;font-size:13px">${safe(booking.id)}</td>
          </tr>
          <tr>
            <td style="padding:10px 14px;border-bottom:1px solid #e3ebe0;color:#5b6a63;font-size:13px">Guest Name</td>
            <td style="padding:10px 14px;border-bottom:1px solid #e3ebe0;font-weight:600;color:#173d35;font-size:13px">${safe(booking.name)}</td>
          </tr>
          <tr>
            <td style="padding:10px 14px;border-bottom:1px solid #e3ebe0;color:#5b6a63;font-size:13px">Service / Villa</td>
            <td style="padding:10px 14px;border-bottom:1px solid #e3ebe0;font-weight:600;color:#173d35;font-size:13px">${safe(booking.service)}</td>
          </tr>
          <tr>
            <td style="padding:10px 14px;border-bottom:1px solid #e3ebe0;color:#5b6a63;font-size:13px">${safe(dateLabel)}</td>
            <td style="padding:10px 14px;border-bottom:1px solid #e3ebe0;font-weight:600;color:#173d35;font-size:13px">${safe(dateValue)}</td>
          </tr>
          <tr>
            <td style="padding:10px 14px;border-bottom:1px solid #e3ebe0;color:#5b6a63;font-size:13px">Guest Count</td>
            <td style="padding:10px 14px;border-bottom:1px solid #e3ebe0;font-weight:600;color:#173d35;font-size:13px">${safe(booking.guests || 2)} Guests</td>
          </tr>
          <tr>
            <td style="padding:10px 14px;color:#5b6a63;font-size:13px">Current Status</td>
            <td style="padding:10px 14px;font-weight:700;color:#dc2626;font-size:13px">Cancelled</td>
          </tr>
        </tbody>
      </table>

      <!-- Need Help / Support -->
      <div style="margin-top:24px;padding:16px 18px;background:#f3f5ef;border-radius:10px;font-size:13px;color:#475569;line-height:1.6">
        <p style="margin:0 0 6px;font-weight:700;color:#173d35">Questions regarding your cancellation or refund?</p>
        <p style="margin:0">
          Please contact our resort team directly at <strong>+91 7083682768</strong> / <strong>+91 ${DEFAULT_CONTACT_PHONE}</strong> or reply directly to this email with your Booking ID (<strong>${safe(booking.id)}</strong>).
        </p>
      </div>

      <div style="margin-top:24px;padding-top:16px;border-top:1px solid #e2e8f0;font-size:12px;color:#94a3b8;text-align:center">
        ${RESORT_NAME} · Countryside Luxury &amp; Nature Stays · <a href="${DEFAULT_MAP_URL}" style="color:#315d4c;text-decoration:none">View Location on Google Maps</a>
      </div>
    </div>
  </div>
</body>
  ...
  if (owners && owners.length) {
    owners.forEach(o => { if (!recipients.includes(o)) recipients.push(o) })
  }

  try {
    const transporter = nodemailer.createTransport({ service: 'gmail', auth: { user, pass: appPassword } })
    await transporter.sendMail({
      from: `${RESORT_NAME} Operations <${process.env.GMAIL_FROM?.trim() || user}>`,
      to: booking.email,
      bcc: owners && owners.length ? owners.join(', ') : undefined,
      subject: `Reservation Cancellation Notice - Booking ${booking.id} (${RESORT_NAME})`,
      html,
    })
    console.log(`[EMAIL:CANCEL_SENT] Cancellation email successfully dispatched to ${booking.email} for booking ${booking.id}`)
    return { sent: true, recipient: booking.email }
  } catch (err) {
    console.error(`[EMAIL:CANCEL_FAILED] Failed sending cancellation email for ${booking.id}:`, err?.message || err)
