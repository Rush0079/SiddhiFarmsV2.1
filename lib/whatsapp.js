...
export function buildWhatsAppMessage(booking, appUrl = '') {
  ...
  const typeHeader = isEvent ? '🎉 *SINGLE-DAY EVENT CELEBRATION*' : (isShortStay ? '🕒 *DAY USE / SHORT STAY (4-5 HOURS)*' : (isDayTour ? '🏊 *DAY TOUR & WATER PARK*' : '🏡 *OVERNIGHT RESORT STAY*'))
  ...
  const lines = [
    ...
    ...(isEvent ? [
      ...
      `• *Short Stay Slot:* *${checkInTime} to ${checkOutTime}* (Day Use Slot)`,
      `• *Guests:* ${booking.guests} Guest(s)`,
    ] : isDayTour ? [
      `• *Visit Date:* ${booking.check_in}`,
