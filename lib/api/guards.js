...
export function cleanPricing(values = {}) {
  const out = PRICING_KEYS.reduce((r, k) => {
    const rawVal = values[k]
    const num = Number(rawVal !== undefined && rawVal !== null ? rawVal : DEFAULT_PRICING[k])
    return {
      ...r,
      [k]: !isNaN(num) && num > 0 ? Math.round(num) : DEFAULT_PRICING[k],
    }
  }, {})
  ...
  for (const [key, label] of Object.entries(rawLabels)) {
    ...
    const extraNum = Number(values[key])
    out[key] = !isNaN(extraNum) && extraNum > 0 ? Math.round(extraNum) : 0
  }
  out._labels = labels
  return out
}
...
export async function requireRole(minRoles, req = null) {
  ...
  // 4. Enforce 2FA Session Verification for all Administrative Mutations
  const requiresAdminRole = minRoles.some((r) => ['staff', 'manager', 'super_admin'].includes(r))
  if (requiresAdminRole && !authHeader.startsWith('Bearer ')) {
    ...
    if (!twoFACookie) {
      try {
        ...
        twoFACookie = cookieStore.get('siddhi_2fa_session')?.value
      } catch {}
    }

    const sessionValid = await verify2FASessionToken(twoFACookie)
    if (!sessionValid || sessionValid.userId !== user.id) {
      console.warn(`[API:AUTH:2FA_REQUIRED] Access blocked for ${user.email} - Missing or expired 2FA session`)
      return { error: 'Unauthorized: Administrative session expired. Please sign in again.', status: 401 }
    }
  }
