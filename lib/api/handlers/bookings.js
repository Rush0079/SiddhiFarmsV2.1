 ...
 * @param {Request} [req]    - Inbound HTTP request for rate limiting.
 * @returns {Promise<NextResponse>}
 */
export async function handleGetPublicBooking(admin, bookingId, req) {
  if (req) {
    const clientIp = getClientIp(req)
    const limit = checkRateLimit(clientIp, 'get_public_booking', 30, 60 * 1000)
    if (!limit.allowed) {
      console.warn(`[API:BOOKINGS:PUBLIC_RATE_LIMIT] IP ${clientIp} exceeded public booking fetch limit`)
      return NextResponse.json(
        { error: `Too many lookup attempts. Please try again in ${limit.resetInSeconds}s.` },
        { status: 429 }
      )
    }
  }
  ...
  const rawEmail = String(booking.email || '').trim()
  let maskedEmail = rawEmail
  if (rawEmail && rawEmail.includes('@')) {
    const [userPart, domainPart] = rawEmail.split('@')
    if (userPart.length <= 2) {
      maskedEmail = `${userPart.slice(0, 1)}***@${domainPart}`
    } else {
      maskedEmail = `${userPart.slice(0, 2)}***${userPart.slice(-1)}@${domainPart}`
    }
  }
