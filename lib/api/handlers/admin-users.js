...
export async function handleAuth2FaSend(admin, body, req) {
  ...
  // Verify caller has authenticated with Supabase password session for this email
  try {
    const supabase = await createSupabaseServerClient()
    const { data: { user } } = await supabase.auth.getUser()
    if (!user || user.email?.toLowerCase() !== email) {
      console.warn(`[API:AUTH:2FA_SEND_BLOCKED] Unauthenticated 2FA request attempt for ${email} from ${clientIp}`)
      return NextResponse.json(
        { error: 'Unauthorized: Valid account sign-in required before requesting verification code' },
        { status: 401 }
      )
    }
  } catch (authErr) {
    console.warn('[API:AUTH:2FA_SESSION_CHECK_ERROR]', authErr.message)
 ...
 * Clears the HTTP-only 2FA session cookie and revokes server Supabase session.
 *
 * @param {Request} [req] - Incoming HTTP request.
 * @returns {Promise<NextResponse>}
 */
export async function handleAuthLogout(req = null) {
  try {
    const supabase = await createSupabaseServerClient()
  ...
  // Invalidate 2FA session cookie
  response.cookies.set('siddhi_2fa_session', '', {
    httpOnly: true,
    secure: process.env.NODE_ENV === 'production',
    sameSite: 'lax',
    path: '/',
  ...
  // Invalidate all Supabase SSR token cookies
