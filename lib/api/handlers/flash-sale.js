...
export async function handleGetFlashSale(admin) {
  ...
  console.log(
    `[API:FLASH_SALE:GET] Flash sale query: ${isLive ? 'LIVE NOW' : 'INACTIVE'} (Enabled: ${sale.enabled}, ServerUTC: ${now.toISOString()})`
  )
...
/**
 * Handles POST /api/flash-sale (Save/schedule promotional flash sale campaign)
 *
 * @param {Object} admin - Supabase admin client.
...
export async function handlePostFlashSale(admin, body, req) {
  ...
  console.log(
    `[API:FLASH_SALE:SAVED] Flash sale updated by ${guard.user.email} (Enabled: ${saleConfig.enabled}, Value: ${saleConfig.discountValue} ${saleConfig.discountType})`
  )
