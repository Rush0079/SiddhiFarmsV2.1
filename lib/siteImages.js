...
export function siteImage(overrides, key) {
  if (key === undefined && typeof overrides === 'string') {
    const slotKey = overrides === 'hero' ? 'homeHero' : overrides
    return IMAGE_DEFAULTS[slotKey] || ''
  }
  const slotKey = key === 'hero' ? 'homeHero' : key
  return (overrides && overrides[slotKey]) || IMAGE_DEFAULTS[slotKey] || ''
}
