...
export { create2FASessionToken, verify2FASessionToken } from './auth-session'
...
export async function verifyOTP({ email, otpCode, purpose }) {
  ...
  const recordBuf = Buffer.from(String(record.hash || ''), 'utf8')
  const expectedBuf = Buffer.from(expectedHash, 'utf8')
  const isMatch = recordBuf.length === expectedBuf.length && crypto.timingSafeEqual(recordBuf, expectedBuf)
