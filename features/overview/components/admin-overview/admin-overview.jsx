...
import { computeWeeklyVelocity, computeStatusDistribution } from '../../models/overview.model'
import { AdminOverviewDefaults } from './admin-overview.model'
...
export default function AdminOverview({
  ...
  const revenueChartData = useMemo(() => computeWeeklyVelocity(bookings), [bookings])
  const statusDistribution = useMemo(() => computeStatusDistribution(bookings, summary), [bookings, summary])
  ...
  return (
    <div className={styles.overviewContainer}>
      ...
      <div className={styles.chartsGrid}>
        ...
        <div className={`stat-card-motion ${styles.chartCard}`}>
          <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
            <div>
              <div className="flex items-center gap-2">
                <TrendingUp size={18} className="text-emerald-700" />
                <p className="eyebrow text-emerald-800">{AdminOverviewDefaults.VELOCITY_SUBTITLE}</p>
              </div>
              <h3 className="font-serif text-xl sm:text-2xl text-[#173d35] mt-1 font-bold">
                {AdminOverviewDefaults.VELOCITY_TITLE}
              </h3>
            </div>
        ...
        <div className={`stat-card-motion ${styles.chartCard} flex flex-col justify-between`}>
          <div>
            <div className="flex items-center gap-2">
              <PieIcon size={18} className="text-emerald-700" />
              <p className="eyebrow text-emerald-800">{AdminOverviewDefaults.HEALTH_SUBTITLE}</p>
            </div>
            <h3 className="font-serif text-xl text-[#173d35] mt-1 font-bold">{AdminOverviewDefaults.HEALTH_TITLE}</h3>
          </div>
      ...
      <div className={styles.activityCard}>
        <div className="flex items-center justify-between border-b border-[#eef2eb] pb-4 mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-serif text-xl font-bold text-[#173d35]">{AdminOverviewDefaults.STREAM_TITLE}</h3>
          </div>
