  ...
  return (
    <div
      className={`flex flex-col items-center justify-center ${
        fullScreen ? 'fixed inset-0 z-50 min-h-screen bg-gradient-to-br from-[#0c2a22] via-[#123830] to-[#071a15]' : 'relative min-h-[400px] w-full bg-transparent'
      } text-white p-6 overflow-hidden`}
    >
      {/* Ambient background glow orbs */}
      <div className="pointer-events-none absolute h-96 w-96 rounded-full bg-[#d5b36a]/10 blur-[100px] animate-pulse" />
      <div className="pointer-events-none absolute h-80 w-80 rounded-full bg-emerald-500/10 blur-[90px]" />
...
export function LuxuryOverlayLoader({
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[100] flex items-center justify-center bg-[#071914]/85 backdrop-blur-xl p-4"
    >
      <motion.div
        initial={{ scale: 0.9, opacity: 0, y: 15 }}
        animate={{ scale: 1, opacity: 1, y: 0 }}
        transition={{ duration: 0.35, ease: 'easeOut' }}
        className="relative overflow-hidden rounded-3xl border border-[#d5b36a]/30 bg-gradient-to-b from-[#0c2a22] to-[#071914] p-8 sm:p-10 text-center shadow-2xl max-w-md w-full"
      >
        {/* Glow behind modal */}
        ...
        <div className="mt-6 inline-flex items-center gap-2 rounded-full bg-black/40 border border-[#d5b36a]/25 px-4 py-1.5 text-xs font-mono text-[#d5b36a]">
