/**
 * ============================================================================
 * BOOKING PANEL COMPONENT — Multi-Step Reservation Form
 * ============================================================================
 *
 * @fileoverview  Full-screen modal booking form handling the entire customer
 *                reservation flow:
 *                1. Service selection (room, tour, event) with stay-type toggle
 *                2. Date/time picker with short-stay presets
 *                3. Coupon/advance code validation (real-time)
 *                4. Razorpay payment gateway integration
 *                5. UPI fallback payment option
 *                6. Post-payment confirmation with WhatsApp share & invoice
 *
 * @module        components/customer/booking-form
 * @author        Rushikesh Nigade
 * @design-pattern Stateful Container — owns all booking/payment flow state
 *                with internal side effects (API calls, Razorpay SDK).
 *
 * @param {Object}      props
 * @param {Object}      props.pricing    - Current pricing rates from Supabase.
 * @param {Object|null} props.user       - Authenticated Supabase user, or null.
 * @param {Function}    props.onClose    - Callback to close the booking modal.
 * @param {Object|null} props.flashSale  - Active flash sale configuration.
 * ============================================================================
 */
'use client'
...
// ─── SHORT STAY DURATION PRESETS ────────────────────────────────────────────
/**
 * Quick-select time slot presets for short stay / day-use bookings.
 * Each tuple: [displayLabel, checkInTime, checkOutTime]
 *
 * @type {Array<[string, string, string]>}
 */
const SHORT_STAY_PRESETS = [
...
// ─── EVENT TIME SLOTS ───────────────────────────────────────────────────────
/**
 * Pre-defined event schedule options for celebrations.
 * Each tuple: [value, displayLabel]
 *
...
/**
 * BookingPanel — Full-screen booking modal with multi-step reservation flow.
 *
 * State machine: idle → booking → (success → paying → paid) | error
 *
 * @param   {Object} props - See @fileoverview for prop descriptions.
 * @returns {JSX.Element} The booking modal overlay.
 */
export default function BookingPanel({ pricing, user, onClose, flashSale }) {
  ...
  /** @type {boolean} True if selected service is a celebration/event */
  const isEvent = ['Engagement Ceremony', 'Birthday Party', 'Get Together', 'Wedding Ceremony'].includes(form.service)
  /** @type {boolean} True if selected service is a day tour */
  const isDayTour = ['One Day Tour', 'Mini Water Park'].includes(form.service)
  /** @type {boolean} True if selected service is a room/villa */
  const isRoom = ['Master Bedroom', '2 BHK Villa', '4 BHK Villa'].includes(form.service)
  /** @type {boolean} True if booking is single-day (no overnight checkout date needed) */
  ...
  /** Calculate subtotal based on service type and stay duration */
  let subtotal = 0
  ...
  /** Auto-apply flash sale discount when no custom coupon is overriding */
  ...
  /**
   * Validates a coupon or advance code against the current subtotal.
   * Handles expiry checks, minimum amount thresholds, and discount calculation.
   *
   * @param {string} code            - The coupon/advance code to validate.
   * @param {number} currentSubtotal - Current booking subtotal for validation.
   */
  async function validateCouponWithSubtotal(code, currentSubtotal) {
    ...
    // Check against loaded coupons first
    ...
    // Fallback: Check if it's an Advance Deposit Code via API
    try {
    ...
    } catch { /* Advance code API unavailable — silently fail */ }

    // No match found
    setAppliedCoupon(null); setAppliedAdvance(null); setDiscount(0)
    console.log('[UI:BookingPanel:COUPON_VALIDATE] No match found for code:', normalizedCode)
  }

  /**
   * Handles coupon input field changes with real-time validation.
  ...
  /**
   * Creates a new booking via the /api/bookings endpoint.
   * Handles all service types (overnight, short-stay, day-tour, event)
   * with appropriate time defaults.
   *
  ...
  async function createBooking(event) {
    ...
    try {
      // Determine check-in/out times based on service type
  ...
  /**
   * Opens the Razorpay payment gateway for the current booking.
   * Handles order creation, SDK initialization, and payment verification.
   */
      ...
      <div className="flex max-h-[calc(100dvh-1rem)] w-full max-w-2xl flex-col overflow-hidden rounded-t-3xl bg-[#fbfaf6] shadow-2xl sm:max-h-[calc(100dvh-3rem)] sm:rounded-3xl border border-[#dfe7dc]/60">
        ...
        /* ─── State: Initial Booking Form ────────────────────────────────── */
          ...
          <form onSubmit={createBooking} className="grid gap-4 sm:grid-cols-2">
            ...
            {/* Room Stay Type Toggle (Overnight vs Short Stay) */}
            {isRoom && (
                  ...
                  <div className="mt-3 pt-3 border-t border-[#eef2eb] space-y-3">
                    <div>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5">
                        {SHORT_STAY_PRESETS.map(([label, inT, outT]) => (
                          <button key={label} type="button"
                            onClick={() => { setShortStayInTime(inT); setShortStayOutTime(outT) }}
                            className={`rounded-lg py-1.5 px-2 text-[11px] font-medium border transition text-center ${shortStayInTime === inT && shortStayOutTime === outT ? 'bg-[#173d35] text-white border-[#173d35] shadow-xs font-bold' : 'bg-white text-slate-700 border-slate-200 hover:bg-[#edf1e8]'}`}>
                            {label.split(' (')[0]}
                          </button>
                        ))}
                      </div>
